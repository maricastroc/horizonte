export type Track = {
  title: string;
  /** Duração do catálogo (segundos). Em curso, `dur` real vem do HTMLAudioElement. */
  dur: number;
...
/** `A—001` → `a-001`, usado para nomear os arquivos de capa e de áudio. */
const slug = (cat: string) => cat.toLowerCase().replace(/[^a-z0-9]+/g, "-");

export const ALBUMS: Album[] = ALBUMS_RAW.map((a) => {
  const id = slug(a.cat);
  return {
    artist: a.artist,
    title: a.title,
    ...
    bpm: a.bpm,
    seed: a.seed,
    inkA: [...a.inkA] as [number, number, number],
    inkB: [...a.inkB] as [number, number, number],
    cover: `/covers/${id}.webp`,
    tracks: a.tracks.map(([title, dur], k) => ({
      title,
      dur,
      src: `/audio/${id}-${String(k + 1).padStart(2, "0")}.m4a`,
    })),
  };
});
